...
std::string Displayer::DisplaySimilarCodes(std::vector<std::pair<Node *, Node *>> similarCodes)
{
	...
	for (auto pair : similarCodes)
	{
		if (pair.first->value().type == "file")
		{
			...
			result += "\n" + std::string(53, '=') + "\n";
			int pos = pair.first->value().name.find_last_of("\\");
			int pos2 = pair.second->value().name.find_last_of("\\");
			result += "\n Comparing File (first) " + pair.first->value().name.substr(pos + 1) + " and File (second) " + pair.second->value().name.substr(pos2 + 1) + "\n\n\n";
		}
		else
		{
			if (pair.first->getComplexity() >= 3)
			{
				std::ostringstream conv,conv2;
				conv << pair.first->getComplexity();
				conv2 << pair.second->getComplexity();
				...
				result += "Pair:\n  " + pair.first->value().showNum() + " Complexity: " + conv.str() + "\n  " + pair.second->value().showNum() + " Complexity= " +conv2.str() + "\n\n";
			}
		}
	}
	return result;
